...
class Figures:
	...
	def get_plot(self, colours: Colours):
		...
		for i, col in enumerate(cols):
			xa,ya=x1+(xlen*i),y1
			currentAxis.add_patch(Rectangle((xa, ya), xlen, ylen,alpha=col[-1], color=col))
			xb,yb=xlen/2+(xlen*i),ylen/3
			plt.text(xb, yb, f'RGBA: {col}\nhex: {mpl.colors.rgb2hex(col, keep_alpha=True)}', 
			backgroundcolor='white', ha='center', va='center', size=text_size, rotation='vertical')
		# set limits
	...
	def print_question(self,choice_path,choice_name):
		...
		if choice_save_cols=='y':
			if [choice_path,choice_name]==[None,None]:
				...
				print("Would you like to use the same path and name for the figure as for the colours?")
				choice_same = input("(y/n): ")
				if choice_same!='y' and choice_same!='n':
						while choice_same!='y' and choice_same!='n':
							choice_same=input("Please enter either 'y' or 'n': ")
				if choice_same=='y':
					self.save_fig(choice_path, choice_name)
				if choice_same=='n':
					self.quest_path_name_corr()
